...
from flask import Blueprint, render_template, request, current_app, send_file, abort, redirect
import io
...
def expand_terms(q, pais=None):
    """Expande queries con contexto e-commerce específico para evitar ruido."""
...
def _fetch_safe(name, ingestor_cls, query, timeout=10, **kwargs):
    """Ejecuta un ingestor con timeout y manejo robusto de parámetros."""
    try:
...
def collect_items_parallel(q, pais="paraguay", max_workers=4):
    """Obtiene items de todas las fuentes en paralelo con control de fallos."""
    items = []
...
def _find_rising_questions(db, topic=""):
    ...
    for question, recent_freq in sorted(recent.items(), key=lambda x: x[1], reverse=True)[:10]:
        ...
        if cambio > 5:
            rising.append({"question": question, "frecuencia_reciente": recent_freq, "crecimiento": f"+{cambio:.0f}%"})
    return rising[:5]
...
def _rank_opportunities(questions, cat_counter, total, cross_items, trend_text):
    ...
    for cat, count in cat_counter.most_common():
        ...
        if score >= 0.7: nivel = 3
        elif score >= 0.5: nivel = 2
        elif score >= 0.3: nivel = 1
        else: nivel = 0
        ranking.append({
            "categoria": cat, "señales": count, "porcentaje": f"{pct:.0f}%", "volumen_pct": pct,
            "dolor": dolor, "solucionabilidad": sol, "tendencia_etq": "al alza" if tendencia > 0 else "a la baja" if tendencia < 0 else "estable",
            "score": round(score, 2), "nivel": nivel, "etiqueta": ETIQUETA_RANKING[nivel], "impacto_negocio": CAT_BUSINESS_IMPACT.get(cat, ""),
        })
...
def build_summary(q, items, questions, db, top_keywords, pais="paraguay"):
    ...
    if trend_text is None:
        trend_text = "Primera corrida de análisis. Estos datos establecen la línea base para futuras comparaciones." if total else "Sin datos históricos."
    ...
    for src, count in src_counter.most_common():
        ...
        if src == "reddit":
            nombre = f"Reddit r/{c.title()}"
        elif src == "mercadolibre":
            nombre = f"MercadoLibre ({c.upper()})"
        elif src == "trends":
            nombre = f"Google Trends ({c.title()})"
        elif src == "x":
            nombre = f"X/Twitter ({c.title()})"
        else:
            nombre = src.title()
        anexo_fuentes.append({
            "clave": src, "nombre": nombre, "señales": count,
            "confiabilidad": SOURCE_RELIABILITY.get(src, "Media"),
            "preguntas": sorted(src_items, key=lambda x: x["confidence"], reverse=True)[:10],
        })
        
    return {
        "period": "últimos 90 días", "topic": q, "pais": pais, "total_questions": total,
        "top_source": top_src, "top_category": top_cat, "categories": dict(cat_counter), "sources": dict(src_counter),
        "top_keywords": top_keywords, "top_items": sorted(questions, key=lambda x: x["confidence"], reverse=True)[:15],
        ...
        "anexo_fuentes": anexo_fuentes, "oportunidades": oportunidades, "rising_questions": rising_questions,
        "exec": exec_summary, "main_finding": exec_summary["que_cambio"], "trend": trend_text,
        "cross_finding": cross_finding, "cross_items": cross_items, "insight": insight_text,
...
def _get_churn(cat, src):
    a = {"logistica_envios":{"reddit":"Publicar thread con tiempos reales + código de seguimiento","review":"Contactar al cliente en <2h, ofrecer compensación y documentar caso","default":"Badge 'Entrega garantizada' en checkout + SMS de confirmación"},"pagos_financiacion":{"reddit":"Crear hilo aclaratorio sobre métodos de pago aceptados + screenshot del proceso","review":"Ofrecer alternativa de pago inmediata y escalar a equipo de finanzas","default":"Mostrar íconos de métodos de pago visibles en header y agregar FAQ de financiación"},"confianza_seguridad":{"reddit":"Responder con certificaciones, garantías y enlace a política de devolución","review":"Escalar a legal, ofrecer reembolso preventivo y publicar caso resuelto","default":"Agregar sección 'Por qué confiar en nosotros' con testimonios verificados"},"experiencia_compra":{"default":"Enviar email post-compra con pasos claros de seguimiento + contacto directo de soporte"}}
    return a.get(cat,{}).get(src, a.get(cat,{}).get("default","Revisar manualmente y priorizar respuesta en <4h"))

def _get_trend(cat, src):
    a = {"logistica_envios":"Crear landing dedicada a 'Envíos a [país]' con calculadora de tiempos y costos","pagos_financiacion":"Lanzar banner promocional de financiación sin intereses + comparar vs competencia","marketing_descubrimiento":"Ajustar meta-ads con keywords de comparación y agregar badge 'Mejor precio'","plataformas_canales":"Crear comparativa visual de canales (web vs app vs marketplace) con ventajas de cada uno","default":f"Crear contenido específico sobre '{cat}' en {src} + monitorear conversión en 48h"}
    return a.get(cat, a["default"])

def _map_kpis(cat, alert_type):
    kpi_map = {
    ...
    return kpi_map.get(cat, {}).get(alert_type, ["KPI genérico", "Monitorear"])

# ✅ Caché interno thread-safe (reemplaza el import roto de trendqa.cache)
_CACHE_STORE = {}
_CACHE_LOCK = Lock()
...
def run_pipeline(q, pais):
    """Pipeline de análisis multi-país. `pais` es obligatorio y ya validado en la ruta."""
...
def dashboard():
    ...
    # ✅ FIX ÚNICO: La clave de caché ahora incluye categoría Y país
...
def serve_report():
    ...
    pdf_content = f"""%PDF-1.4 1 0 obj<</Type/Catalog/Pages 2 0 R>>endobj 2 0 obj<</Type/Pages/Kids[3 0 R]/Count 1>>endobj 3 0 obj<</Type/Page/MediaBox[0 0 612 792]/Parent 2 0 R/Resources<</Font<</F1 4 0 R>>>>/Contents 5 0 R>>endobj 4 0 obj<</Type/Font/Subtype/Type1/BaseFont/Helvetica>>endobj 5 0 obj<</Length 200>>stream BT /F1 16 Tf 50 700 Td (TrendQA - {topic}) Tj 0 -20 Td ({pais}) Tj 0 -20 Td (SIKURI) Tj ET endstream endobj xref 0 6 0000000000 65535 f 0000000015 00000 n 0000000060 00000 n 0000000115 00000 n 0000000270 00000 n 0000000340 00000 n trailer<</Size 6/Root 1 0 R>> startxref 590 %%EOF""".encode('latin-1')
